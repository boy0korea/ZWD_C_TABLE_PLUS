...
CLASS ZCL_WD_C_TABLE_PLUS IMPLEMENTATION.
...
*CL_FPM_TREE_RENDERING
    ...
    DATA: lo_header_menu      TYPE REF TO cl_wd_menu,
          ...
          lv_path_text        TYPE string,
          lv_path_image       TYPE string,
          lv_path_source      TYPE string.

    CHECK mo_c_table IS BOUND.
    lo_view = mo_c_table->get_view( ).
    CHECK lo_view IS BOUND.

    lo_header_menu = cl_wd_menu=>new_menu( id      = |{ mo_c_table->id }___{ iv_column_id }_MNU_FPM_COLUMN_HEADER|
                                           view    = lo_view
                                           visible = abap_true ).
    ...
    ls_parameter-name = 'COLUMN'.                           "#EC NOTEXT
    ls_parameter-value = iv_column_id.
    APPEND ls_parameter TO lt_parameter.

    ls_parameter-name = 'DIRECTION'.                        "#EC NOTEXT
    ls_parameter-value = cl_wd_c_table_column=>e_sort_state-up.
    APPEND ls_parameter TO lt_parameter.

    lo_menu_action_item ?= lo_menu_item.
    lo_menu_action_item->map_on_action( lt_parameter ).
    lo_header_menu->add_item( lo_menu_item ).
    ...
    ls_parameter-name = 'DIRECTION'.                        "#EC NOTEXT
    ls_parameter-value = cl_wd_c_table_column=>e_sort_state-down.
    APPEND ls_parameter TO lt_parameter.
  ...
  METHOD create_mt_column_mapping.
    DATA: lt_column         TYPE cl_wd_abstr_c_table_column=>tt_abstr_c_table_column,
          ...
          lo_header_menu    TYPE REF TO cl_wd_menu,
...
*CL_FPM_TREE_ASSISTANCE
  ...
  METHOD dispatch_export.
    DATA: lt_field2         TYPE za2xh_t_fieldcatalog,
          ...
          lo_data_t_type    TYPE REF TO cl_abap_tabledescr,
          lr_data           TYPE REF TO data.
    FIELD-SYMBOLS: <lt_data> TYPE table,
                   <ls_data> TYPE data,
                   <lv_data> TYPE data.

    CHECK: zcl_abap2xlsx_helper=>is_abap2xlsx_installed( iv_with_message = abap_false ) EQ abap_true.

    lo_node_info = mo_c_table->get_data_source( )->get_node_info( ).
    lo_data_s_type = lo_node_info->get_static_attributes_type( ).
    lo_data_t_type = cl_abap_tabledescr=>create( lo_data_s_type ).
    CREATE DATA lr_data TYPE HANDLE lo_data_t_type.
    ...
    LOOP AT mt_column_mapping INTO ls_column_mapping.
      ...
      READ TABLE lt_field2 INTO ls_field WITH KEY fieldname = ls_column_mapping-table_field_name.
      CHECK: sy-subrc EQ 0.

      ls_field-label_text = ls_column_mapping-ui_column->get_header( )->get_text( ).
      ls_field-fixed_values = lo_node_info->get_attribute( ls_column_mapping-table_field_name )-value_set.
  ...
  METHOD dispatch_sort_event.
    DATA: lv_sort_column_name TYPE string,
          lv_sort_direction   TYPE string,
          lv_sort_descending  TYPE flag.

    lv_sort_column_name = io_event->get_string( if_fpm_guibb_tree=>gc_event_par_column ).
    lv_sort_direction = io_event->get_string( if_fpm_guibb_tree=>gc_event_par_direction ).
    CASE lv_sort_direction.
      WHEN cl_wd_c_table_column=>e_sort_state-up.
        lv_sort_descending = abap_false.
      WHEN cl_wd_c_table_column=>e_sort_state-down.
        lv_sort_descending = abap_true.
    ENDCASE.

    CAST cl_wdr_c_table_method_hndl( mo_c_table->_method_handler )->sort_table(
      EXPORTING
        column_id        = lv_sort_column_name
      ...
      WHEN gc_search_state-to_be_executed.
        ...
        IF lr_current_hit IS BOUND.
          ...
          update_first_visi_row( EXPORTING ir_current_hit = lr_current_hit
                                           iv_scroll_type = gc_scroll_type-jump_to_initial_hit ).
        ELSE.
          clear_front_end( ).
        ENDIF.
        " ------------------------------------------------------------
      WHEN gc_search_state-jump_to_next.
        ...
        IF lr_current_hit IS BOUND.
          ...
          IF lv_start_over = abap_false.
            update_first_visi_row( EXPORTING ir_current_hit = lr_current_hit
                                             iv_scroll_type = gc_scroll_type-jump_to_next ).
          ELSE.
            update_first_visi_row( EXPORTING ir_current_hit = lr_current_hit
                                             iv_scroll_type = gc_scroll_type-jump_to_prev ).
          ENDIF.
        ENDIF.
        mv_user_changed_data = abap_false.
        " ------------------------------------------------------------
    ...
    IF sy-subrc <> 0.
      ...
      lo_wdr_view->add_action(
        EXPORTING
          action  = NEW cl_wdr_action(
          controller       = lo_v
          text_key         = 'DISPATCH_SORT'
          event_handler    = 'ONACTIONDISPATCH_SORT'
        )
      ...
      lo_wdr_view->add_action(
        EXPORTING
          ...
          controller       = lo_v
          text_key         = 'EXPORT'
          event_handler    = 'ONACTIONEXPORT'
        )
          command = 'ZWD_C_TABLE_PLUS_EXPORT'
      ).
...
*CL_FPM_TREE_ASSISTANCE
    ...
    CASE iv_scroll_type.
        " -------------------------------------------------------------
      WHEN gc_scroll_type-jump_to_next.
        " -------------------------------------------------------------
        ...
        ENDIF.
        " -------------------------------------------------------------
      WHEN gc_scroll_type-jump_to_prev.
        " -------------------------------------------------------------
        lv_first_visi_row = get_first_visi_row( ).
        ...
        ENDIF.
        " -------------------------------------------------------------
      WHEN gc_scroll_type-jump_to_initial_hit.
        " -------------------------------------------------------------
        " get upper boundary
        lv_first_visi_row = get_first_visi_row( ).
        " get lower boundary
...
* 검색 버튼을 눌러서 테이블 내용이 변경되는 경우 정렬표시를 제거함.
    ...
    on_search_close( ).
    IF mv_sort_column_name IS NOT INITIAL.
